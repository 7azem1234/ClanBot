...
module.exports = {
    name: 'clanpromote',
    description: "Promote Player in your clan",
    cooldown: 3000,
    options: [
        {
            name: 'member',
            description: "The Member you want to promote on your clan",
            type: 6,
            required: true
        }
    ],
    ...
    run: async (client, interaction) => {
        ...
        if (!UserLeaderRole) {//check if user is leader
        ...
        if (target.user.id === interaction.member.user.id) {//check if target == user
            const c = new EmbedBuilder()
                .setColor('#ff0000')
                .setTitle('FoxMC Clans')
                .setDescription("> You can't promote yourself!")
                .setTimestamp()
                .setFooter({ text: 'FoxMCClans', iconURL: client.user.displayAvatarURL() });
            try{
            ...
            switch (role.id) {
                case GeneralRole:
                    const e = new EmbedBuilder()
                        .setColor('#ff0000')
                        .setTitle('FoxMC Clans')
                        .setDescription("> You can't promote a General in your clan!")
                        .setTimestamp()
                        .setFooter({ text: 'FoxMCClans', iconURL: client.user.displayAvatarURL() });
                    try{
                    ...
                    break;
                case OfficerRole:
                    target.member.roles.add(GeneralRole).then(() => {
                        try{
                            target.member.roles.remove(role)
                        }catch (err){
                            console.log(err)
                        }
                        UpdateData(GeneralRole)
                        const f = new EmbedBuilder()
                            .setColor('#00FF00')
                            .setTitle('FoxMC Clans')
                            .setThumbnail(target.member.displayAvatarURL())
                            .setDescription(`> ${target.user.username} has been successfully promoted to Clan General`)
                            .setTimestamp()
                            .setFooter({ text: 'FoxMCClans', iconURL: client.user.displayAvatarURL() });
                        try{
                            interaction.reply({ embeds: [f] })
                            console.log(`${target.user.id} (${target.user.username}) has been successfully promoted to Clan General`)
                        }catch (err){
                            console.log(err)
                        }
                    })
                ...
                case VeteranRole:
                    target.member.roles.add(OfficerRole).then(() => {
                        ...
                        const k = new EmbedBuilder()
                            ...
                            .setTitle('FoxMC Clans')
                            .setThumbnail(target.member.displayAvatarURL())
                            .setDescription(`> ${target.user.username} has been successfully promoted to Clan Officer`)
                            .setTimestamp()
                            .setFooter({ text: 'FoxMCClans', iconURL: client.user.displayAvatarURL() });
                        try{
                            interaction.reply({ embeds: [k] })
                            console.log(`${target.user.id} (${target.user.username}) has been successfully promoted to Clan Officer`)
                        }catch (err){
                            console.log(err)
                        }
                    })
                ...
                case MemberRole:
                    target.member.roles.add(VeteranRole).then(() => {
                        ...
                        const y = new EmbedBuilder()
                            ...
                            .setTitle('FoxMC Clans')
                            .setThumbnail(target.member.displayAvatarURL())
                            .setDescription(`> ${target.user.username} has been successfully promoted to Clan Veteran`)
                            .setTimestamp()
                            .setFooter({ text: 'FoxMCClans', iconURL: client.user.displayAvatarURL() });
                        try{
                            interaction.reply({ embeds: [y] })
                            console.log(`${target.user.id} (${target.user.username}) has been successfully promoted to Clan Veteran`)
                        }catch (err){
                            console.log(err)
                        }
                    })
